...
units.P_mmHg2kPa = P_mmHg2kPa; 
units.V_mL2m3    = V_mL2m3; 
...
SPbar = SPbar * P_mmHg2kPa;    
DPbar = DPbar * P_mmHg2kPa; 
Pbar  = Pbar  * P_mmHg2kPa; 

% Convert from mL to m^3
Vtot   = Vtot   * V_mL2m3; 
SV_nom = SV_nom * V_mL2m3; 
EDV_LV = EDV_LV * V_mL2m3; 
ESV_LV = ESV_LV * V_mL2m3; 

% Convert from mL min^(-1) to m^3 s^(-1)
CO_nom = CO_nom * V_mL2m3 / 60;     

% Convert from mmHg min ml^(-1) to kPa s m^(-3)
SVR_nom = Pbar * P_mmHg2kPa / CO_nom; 
...
E_lv = outputs.elastances.E_lv; 
E_rv = outputs.elastances.E_rv; 
