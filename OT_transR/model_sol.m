...
volumes.V_ao = (sols(10,:) + V_aou)  * 1e6; 
volumes.V_sa = (sols(11,:) + V_sau) * 1e6; 
volumes.V_sv = (sols(12,:) + V_svu) * 1e6; 
volumes.V_ra = sols(13,:) * 1e6; 
volumes.V_rv = sols(14,:) * 1e6; 
volumes.V_pa = (sols(15,:) + V_pau) * 1e6; 
volumes.V_pv = (sols(16,:) + V_pvu) * 1e6; 

% Convert kPa to mmHg
pressures.P_la = o(1,:) * 7.5; 
pressures.P_lv = o(2,:) * 7.5; 
pressures.P_ao = o(3,:) * 7.5; 
pressures.P_sa = o(4,:) * 7.5; 
pressures.P_sv = o(5,:) * 7.5; 
pressures.P_ra = o(6,:) * 7.5; 
pressures.P_rv = o(7,:) * 7.5; 
pressures.P_pa = o(8,:) * 7.5; 
pressures.P_pv = o(9,:) * 7.5; 

% Convert m^3 to cm^3
wallvolumes.Vm_lv  = o(10,:)  * 1e6; 
wallvolumes.Vm_sep = o(11,:)  * 1e6; 
wallvolumes.Vm_rv  = o(12,:) * 1e6; 

% Convert m^2 to cm^2
areas.Am_lv  = o(13,:) * 1e4; 
areas.Am_sep = o(14,:) * 1e4; 
areas.Am_rv  = o(15,:) * 1e4; 

% Convert m^(-1) to cm^(-1)
curvatures.Cm_lv  = o(16,:) * 1e-2;
curvatures.Cm_sep = o(17,:) * 1e-2;
curvatures.Cm_rv  = o(18,:) * 1e-2; 

strains.eps_lv  = o(19,:); 
strains.eps_sep = o(20,:); 
strains.eps_rv  = o(21,:); 

stresses.passive.sigma_pas_lv  = o(22,:);
stresses.passive.sigma_pas_sep = o(23,:);
stresses.passive.sigma_pas_rv  = o(24,:);

stresses.active.sigma_act_lv  = o(25,:);
stresses.active.sigma_act_sep = o(26,:);
stresses.active.sigma_act_rv  = o(27,:);

stresses.total.sigma_lv  = o(28,:);
stresses.total.sigma_sep = o(29,:);
stresses.total.sigma_rv  = o(30,:);

% Convert m^3 s^(-1) to L min^(-1)
flows.Q_m_valve = o(31,:) * 1e3 * 60; 
flows.Q_a_valve = o(32,:) * 1e3 * 60; 
flows.Q_t_valve = o(33,:) * 1e3 * 60; 
flows.Q_p_valve = o(34,:) * 1e3 * 60; 

flows.Q_ao = o(35,:) * 1e3 * 60; 
flows.Q_sa = o(36,:) * 1e3 * 60; 
flows.Q_sv = o(37,:) * 1e3 * 60; 
flows.Q_pa = o(38,:) * 1e3 * 60; 
flows.Q_pv = o(39,:) * 1e3 * 60; 
