...
pars = parameters2(exercise0,HF,theta,HFpEFdata,data);
...
fun = @(x) model_wrap_baseline_HFpEF(x,exercise0,data,HFpEFdata); 
...
%     'MaxGenerations',10); 
% popt = ga(fun,length(p0),[],[],[],[],lb,ub,[],options);

% options = optimset('Display','iter'); 
% popt = fminsearch(fun,p0,options); 

%% Solve optimized model 

pars_opt = parameters(exercise0,popt,theta); 
