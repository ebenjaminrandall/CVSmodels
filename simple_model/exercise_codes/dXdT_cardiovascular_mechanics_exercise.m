...
% C_SA = 0.65*1.65/(1 + d*theta); % Systemic arterial compliance, mL/mmHg
...
% R_SA   = 1.5*0.965/(1 + b*theta);% mmHg*sec/mL; % Systemic vasculature resistance, mmHg*sec/mL
...
% Normal
sigmapas_LV  = (22) * (SLo_LV  - 1.6)  + 22*max(0,SLo_LV-1.6)^3 ;
sigmapas_SEP = (22) * (SLo_SEP  - 1.6) + 22*max(0,SLo_SEP-1.6)^3 ;
sigmapas_RV  = (22) * (SLo_RV  - 1.6) + 22*max(0,SLo_RV-1.6)^3 ;

% % HFpEF
% sigmapas_LV  = 6*(22) * (SLo_LV  - 1.6)  + 5*22*max(0,SLo_LV-1.8)^3 ;
% sigmapas_SEP = 6*(22) * (SLo_SEP  - 1.6) + 5*22*max(0,SLo_SEP-1.8)^3 ;
% sigmapas_RV  = 6*(22) * (SLo_RV  - 1.6) + 5*22*max(0,SLo_RV-1.8)^3 ;

% Active forces
sigmaact_LV  = sigma_act*C_LV*(SL_LV-1.6)*(SLo_LV - SL_LV)/LSEiso;
sigmaact_SEP = sigma_act*C_SEP*(SL_SEP-1.6)*(SLo_SEP - SL_SEP)/LSEiso;
sigmaact_RV  = sigma_act*C_RV*(SL_RV-1.6)*(SLo_RV - SL_RV)/LSEiso;
...
QIN_RA = (P_SV - P_RA)/R_RA*(P_SV>P_RA) + (P_SV - P_RA)/R_RA*(P_SV<P_RA)*(V_RA>0);
QIN_LA = (P_PV - P_LA)/R_LA*(P_PV>P_LA) + (P_PV - P_LA)/R_LA*(P_PV<P_LA)*(V_LA>0);
